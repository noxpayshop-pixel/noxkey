...
import { MessageCircle, Loader2, ShieldCheck, AlertTriangle } from 'lucide-react';

const DiscordLoginPanel = ({ onSuccess }: { onSuccess?: () => void }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">Login with Discord</h2>
          <p className="text-sm text-muted-foreground">We'll send you a code via DM</p>
        </div>
      </div>
          ...
          <motion.div key="username" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            {notInServer && (
              <div className="flex items-start gap-3 bg-card rounded-xl p-4 border border-border">
                ...
                <div>
                  <p className="text-sm text-foreground font-medium">You must join our Discord first!</p>
                  <p className="text-xs text-muted-foreground mt-1">Join the server, then try logging in again.</p>
                  <a
